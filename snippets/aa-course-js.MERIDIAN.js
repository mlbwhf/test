...
(function(){
	...
	// ==================== MERIDIAN AGENDA (light "Upcoming sessions" card) ====================
	// Renders ALL upcoming items into #aa-agenda. No-ops on any page lacking that host.
	// Reuses the SAME per-item data + registration wiring as buildRail (openReg/selectCohort).
	function buildAgenda(items){
		var host = $('aa-agenda');
		if(!host) return;                                          // no agenda on this page -> do nothing
		var cfg = $('aa-cohorts');
		var D = (cfg && cfg.dataset) || {};
		var price = (D.price || '').toString();
		...
		for(var i=0; items.length > i; i++){
			...
			var mf  = MONF[MON.indexOf(it.mon)] || it.mon;
			var full = (WD[dow] ? WD[dow] + ', ' : '') + it.day + ' ' + mf;
			var ff   = it.mon + ' ' + it.day + (it.when ? (' — ' + it.when) : '');
			var time = it.time || TIME_DEFAULT;
			var ebHref = (it.href && it.href.charAt(0) !== '#') ? it.href : (it.eb || EB_ORG);
			var pill = i === 0
				? '<span class="aa-agenda-pill hot">FAST FILLING</span>'
				: (i === 3 ? '<span class="aa-agenda-pill low">FEW SEATS</span>' : '');
			var meta = esc(time) + ' · ' + length + ' DAYS · $' + esc(price);
			rows += '<div class="aa-agenda-row">' +
		...
		host.className = 'aa-agenda';
		host.innerHTML =
			'<div class="aa-agenda-head">' +
				'<span class="aa-agenda-title">Upcoming sessions &middot; ' + items.length + ' dates</span>' +
				'<span class="aa-agenda-tz">All times ET</span>' +
			'</div>' +
			'<div class="aa-agenda-list">' + rows + '</div>' +
			'<div class="aa-agenda-foot">&#128274; Register &amp; pay securely &middot; Stripe / Eventbrite &middot; Instant confirmation</div>';
		var regs = host.querySelectorAll('.aa-agenda-reg');
		Array.prototype.forEach.call(regs, function(a){
			a.addEventListener('click', function(e){
				e.preventDefault();
				selectCohort(a.getAttribute('data-ff'));
				openReg();
			});
		});
	}
