<?php
/**
 * Template Name: RentNova Blueprint
 *
 * Drop-in landing-page template for any existing WordPress theme.
 * Self-contained: all CSS and JS are scoped under `.rnbp` and printed
 * inline, so it does NOT depend on your active theme's styles and will
 * not leak styles into the rest of your site.
 *
 * HOW TO USE
 * 1. Copy this file into your active theme (or child theme) folder,
 *    e.g. wp-content/themes/<your-theme>/template-rentnova-blueprint.php
 * 2. WordPress admin → Pages → Add New.
 * 3. In Page Attributes → Template, choose "RentNova Blueprint".
 * 4. Publish. The page renders inside your existing header/footer.
 *
 * Edit copy/images directly in the markup below, or wire them to ACF /
 * page meta if you prefer. Swap the two stay image URLs for your own.
 *
...
// Load the design fonts (safe to enqueue inline here for a single page).
?>
...
<style>
/* ===== RentNova Blueprint — scoped drop-in styles ===== */
.rnbp{
  /* full-bleed: break out of theme content containers */
...
.rnbp__in{ max-width:var(--rn-max); margin:0 auto; }
...
.rnbp__hero h1{ font-family:'Archivo',sans-serif; font-size:74px; font-weight:900; line-height:.9; letter-spacing:-.04em; margin:22px 0; }
...
/* stays */
.rnbp__stays{ display:grid; grid-template-columns:1fr 1fr; gap:24px; }
...
.rnbp__stay-img{ width:100%; height:240px; object-fit:cover; background:var(--rn-paper2); }
...
.rnbp__stayslink{ font-size:15px; font-weight:700; color:var(--rn-terra); }
...
@media (max-width:860px){
  ...
  .rnbp__stays{ grid-template-columns:1fr; }
...
<div class="rnbp">
	...
	<section class="rnbp__hero">
		<div class="rnbp__hgrid">
			<div>
				<div class="rnbp__eyebrow">Turn your residence into income</div>
				<h1>Make your<br>home <em>pay.</em></h1>
				<p class="rnbp__lead">One home into two or three legal income units. We design, build, furnish, list and run them — one team, one contract, monthly statements.</p>
				<div class="rnbp__actions">
					<a class="rnbp__btn rnbp__btn--terra" href="#rnbp-contact">Make your home pay →</a>
					<a class="rnbp__btn rnbp__btn--ghost" href="#rnbp-stays">Browse stays</a>
				</div>
			</div>
			...
			<div class="rnbp__bp">
				...
				<svg viewBox="0 0 1200 730" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Architectural section: one lot converted into three legal units — basement suite, main residence, detached garden suite.">
					<defs>
						<marker id="rnbpArr" markerWidth="9" markerHeight="9" refX="4.5" refY="4.5" orient="auto"><path d="M1,1 L8,4.5 L1,8" fill="none" stroke="#cdd6cc" stroke-width="1"/></marker>
						<pattern id="rnbpHb" width="9" height="9" patternTransform="rotate(45)" patternUnits="userSpaceOnUse"><line x1="0" y1="0" x2="0" y2="9" stroke="#eae5d6" stroke-width="1" stroke-opacity="0.30"/></pattern>
						<pattern id="rnbpHm" width="11" height="11" patternTransform="rotate(45)" patternUnits="userSpaceOnUse"><line x1="0" y1="0" x2="0" y2="11" stroke="#eae5d6" stroke-width="1" stroke-opacity="0.16"/></pattern>
						<pattern id="rnbpHg" width="8" height="8" patternTransform="rotate(-45)" patternUnits="userSpaceOnUse"><line x1="0" y1="0" x2="0" y2="8" stroke="#e0a583" stroke-width="1" stroke-opacity="0.28"/></pattern>
						<pattern id="rnbpEarth" width="22" height="22" patternUnits="userSpaceOnUse"><path d="M0,22 L22,0 M-4,4 L4,-4 M18,26 L26,18" stroke="#eae5d6" stroke-width="0.8" stroke-opacity="0.10"/></pattern>
					</defs>
					<rect x="180" y="520" width="760" height="150" fill="url(#rnbpEarth)" style="opacity:0;animation:rnbpIn 1s ease forwards 1.1s;"/>
					<rect x="260" y="520" width="360" height="120" fill="url(#rnbpHb)" style="opacity:0;animation:rnbpIn .8s ease forwards 1.15s;"/>
					<rect x="260" y="270" width="360" height="250" fill="url(#rnbpHm)" style="opacity:0;animation:rnbpIn .8s ease forwards 1.3s;"/>
					<path d="M690,520 L690,410 L775,360 L860,410 L860,520 Z" fill="url(#rnbpHg)" style="opacity:0;animation:rnbpIn .8s ease forwards 1.45s;"/>
					...
					<g stroke="#cdd6cc" fill="none" stroke-width="1" style="opacity:0;animation:rnbpUp .6s ease forwards 2.3s;">
						<line x1="206" y1="150" x2="206" y2="640" marker-start="url(#rnbpArr)" marker-end="url(#rnbpArr)"/>
						...
						<line x1="260" y1="688" x2="620" y2="688" marker-start="url(#rnbpArr)" marker-end="url(#rnbpArr)"/>
						...
						<line x1="690" y1="556" x2="860" y2="556" marker-start="url(#rnbpArr)" marker-end="url(#rnbpArr)" stroke-opacity="0.75"/>
					</g>
	...
	<!-- FEATURED STAYS -->
	<section class="rnbp__sec" id="rnbp-stays">
		<div class="rnbp__head">
			<h2 class="rnbp__h2" style="font-size:38px;max-width:560px;">The homes we built, available to book.</h2>
			<a class="rnbp__stayslink" href="#">View all stays →</a>
		</div>
		<div class="rnbp__stays">
			<a class="rnbp__stay" href="#">
				<img class="rnbp__stay-img" src="https://rent-nova.com/wp-content/uploads/2026/05/6019-featherhead-crescent-mississauga-W4633966-1.jpg" alt="The Two-Story Home, Erin Mills" loading="lazy" />
				<div class="rnbp__stay-body">
					<div>
						<div class="rnbp__stay-loc">Erin Mills · Mississauga</div>
						<div class="rnbp__stay-name">The Two-Story Home</div>
						<div class="rnbp__stay-meta">4BR · Sleeps 6 · Min 15 nights</div>
					</div>
					<div class="rnbp__stay-price"><b>$167</b><span>/ night</span></div>
				</div>
			</a>
			<a class="rnbp__stay" href="#">
				<img class="rnbp__stay-img" src="https://rent-nova.com/wp-content/uploads/2026/05/Bedroom-1-scaled.jpg" alt="The Erin Mills Suite" loading="lazy" />
				<div class="rnbp__stay-body">
					<div>
						<div class="rnbp__stay-loc">Erin Mills · Mississauga</div>
						<div class="rnbp__stay-name">The Erin Mills Suite</div>
						<div class="rnbp__stay-meta">1BR · Sleeps 2 · Min 3 nights</div>
					</div>
					<div class="rnbp__stay-price"><b>$65</b><span>/ night</span></div>
				</div>
			</a>
		</div>
	</section>
	...
	<!-- CTA -->
	<section class="rnbp__cta" id="rnbp-contact">
...
<script>
( function () {
	var bp = document.querySelector( '.rnbp__bp svg' );
	if ( bp && 'IntersectionObserver' in window && ! window.matchMedia( '(prefers-reduced-motion: reduce)' ).matches ) {
		...
		var io = new IntersectionObserver( function ( entries ) {
			entries.forEach( function ( e ) { if ( e.isIntersecting ) { replay( bp ); } } );
		}, { threshold: 0.35 } );
		io.observe( bp );
	}
} )();
