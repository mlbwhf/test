...
|---|---|---|
| **A1** Registration truth dashboard | ✅ Script built + token supplied 2026-07-05. ⛔ Backfill **cannot run in this environment** — the egress network policy blocks `www.eventbriteapi.com` (proxy returned 403 `connect_rejected`). | **Mark:** either run the script on a machine with open internet (`EVENTBRITE_PRIVATE_TOKEN=… python3 a1_registration_report.py --weeks 12`) and share `reports/`, OR allow `www.eventbriteapi.com` in the environment's network policy so it can run here. Also **rotate the token** — it was pasted in chat. |
