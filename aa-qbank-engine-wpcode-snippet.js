/* ============================================================
 * WPCode Snippet: "AA — QBank Assessment Engine"
 * Type: JavaScript Snippet (WPCode outputs it inside <script> tags)
 * Insertion: Auto Insert — Site Wide Footer
 * ------------------------------------------------------------
 * One engine for all question-bank assessments (innovation-framework,
 * mutation-readiness, agile-assessment, ai-assessment,
 * product-management-assessment). Each page selects an assessment with a
 * small JSON config placed next to the #aa-app mount element:
 *
 *   <script type="application/json" class="aa-qbank-cfg">
 *     {"key":"mutation-readiness"}
 *   <\/script>
 *
 * Full config schema:
 *   {
 *     "key":      "<registry key>",            // required — which assessment runs
 *     "registry": { ...per-key override... },  // optional — DEEP-MERGED over the
 *                                              // built-in registry entry for "key"
 *                                              // (objects merge recursively; strings,
 *                                              // numbers and arrays replace wholesale)
 *     "ui":       { key: "translated string" } // optional — SHALLOW-merged over the
 *                                              // engine's UI string table (chrome:
 *                                              // buttons, headings, gate/trial copy).
 *                                              // Values may keep {n} {total} {title}
 *                                              // {read} {rationale} placeholder tokens.
 *   }
 * Translated pages ship "registry" + "ui" — change labels, not functions.
 * On pages with no .aa-qbank-cfg element the engine no-ops.
 * ============================================================ */
(function(){
/* --- QBank engine bootstrap: read the per-page JSON config --- */
var QBANK_CFG_EL = document.querySelector('script.aa-qbank-cfg');
if(!QBANK_CFG_EL) return; /* no config on this page — engine stays inert */
var QBANK_CFG = {};
try { QBANK_CFG = JSON.parse(QBANK_CFG_EL.textContent || '{}') || {}; } catch(e) { QBANK_CFG = {}; }
const ASSESSMENTS = ({
...
/* --- QBank engine: deep-merge per-page registry override over the built-in entry --- */
function qbankMerge(base, over){
  for (var k in over) {
    if (!Object.prototype.hasOwnProperty.call(over, k)) continue;
    var v = over[k];
    if (v && typeof v === 'object' && !Array.isArray(v) && base[k] && typeof base[k] === 'object' && !Array.isArray(base[k])) { qbankMerge(base[k], v); }
    else { base[k] = v; } /* strings/numbers/arrays replace wholesale */
  }
  return base;
}
if (QBANK_CFG.registry && typeof QBANK_CFG.registry === 'object' && QBANK_CFG.key) {
  if (ASSESSMENTS[QBANK_CFG.key]) { qbankMerge(ASSESSMENTS[QBANK_CFG.key], QBANK_CFG.registry); }
  else { ASSESSMENTS[QBANK_CFG.key] = QBANK_CFG.registry; }
}
/* --- QBank engine: UI chrome strings (overridable via config "ui") --- */
const UI = ({
  pillFreeLive:'Free · Live',
  cardMinutes:'{n} min',
  dimensionsCount:'{n} dimensions',
  questionsCount:'{n} questions',
  startBtn:'Start &rarr;',
  notifyBtn:'Notify me &rarr;',
  landingEyebrow:'Free diagnostics',
  landingH1:'Where does your organization actually stand?',
  landingSub:'{n} live diagnostics built by Mark Saymen (Gold SPCT, author of <em>Mutation Readiness</em>). Honest scoring. Tailored 30-60-90 day plan. Zero fluff.',
  startNowBtn:'Start now &rarr;',
  trust1:'5-15 min each',
  trust2:'No login to start',
  trust3:'Personalized roadmap',
  landingUpsellH:'Want the full diagnostic for every assessment?',
  landingUpsellP:'14-day free trial unlocks the 30-60-90 day action plan, recommended reading per band, and PDF download across all assessments.',
  trialCtaBtn:'Start the 14-day free trial &rarr;',
  unknownAssessment:'Unknown assessment',
  backToAll:'Back to all assessments',
  resultBands:'3 result bands',
  introMinutes:'~{n} min',
  howItWorksLabel:'How it works:',
  howItWorksBody:'Each question accepts a short free-text answer. We\'ll suggest a 1-5 score based on what you wrote — you can override. Honest answers produce useful diagnostics; rosy answers produce rosy lies. No login required to start.',
  allAssessments:'All assessments',
  beginBtn:'Begin &rarr;',
  calibration:'Calibration',
  stepOf:'Step {n} of {total}',
  backBtn:'&larr; Back',
  crunching:'Crunching your answers...',
  qOf:'Q {n} of {total}',
  answerPlaceholder:'Type your honest answer here...',
  reflectLine:'I\'d {read} — {rationale}. Override below if needed.',
  reflectNa:'read this as <strong>N/A</strong>',
  reflectScore:'score that a <strong>{n}</strong>',
  naBtn:'N/A',
  scaleMin:'Never',
  scaleMax:'Always',
  nextBtn:'Next &rarr;',
  submitBtn:'Submit &rarr;',
  noAssessment:'No assessment.',
  noResultsH:'No results yet',
  noResultsP:'Take the {title} first.',
  startAssessmentBtn:'Start the assessment &rarr;',
  outOf:'out of {n}',
  dimHeading:'How you scored by dimension',
  diagnosticHint:'Diagnostic hint:',
  shapeHeading:'Your shape, at a glance',
  meaningHeading:'What your score means',
  plan30:'30 days',
  plan60:'60 days',
  plan90:'90 days',
  readingHeading:'Recommended reading',
  downloadPdf:'Download PDF',
  gateEyebrow:'Unlock full report',
  gateH:'Get your 30-60-90 day action plan',
  gateP:'Personalized roadmap, recommended reading, PDF download, longitudinal tracking — all unlock with the 14-day free trial. No credit card.',
  trialBtn:'Start 14-day free trial &rarr;',
  talkToMark:'Or talk to Mark',
  afterTrial:'After trial',
  priceAmount:'$149',
  pricePer:'/year · all assessments',
  priceB1:'Full 30-60-90 plan per band',
  priceB3:'PDF report',
  priceB4:'Longitudinal tracking',
  priceB5:'Email + advisor support',
  donePrompt:'Done with this one?',
  tryNext:'Try {title} &rarr;',
  gateDoneH:'All done!',
  gateDoneP:'Your {title} is scored and ready. Drop your email to see the results — and we\'ll send a copy to your inbox.',
  emailLabelGate:'Your work email',
  nameLabelGate:'Your first name (optional)',
  emailPlaceholder:'you@yourcompany.com',
  namePlaceholder:'Alex',
  seeResultsBtn:'See my results &rarr;',
  noSpam:'No spam. Unsubscribe anytime.',
  trialModalH:'Start your 14-day free trial',
  trialModalP:'Unlocks the 30-60-90 day plan, recommended reading, PDF download, and longitudinal tracking. No credit card.',
  emailLabel:'Work email',
  nameLabel:'First name (optional)',
  activateTrialBtn:'Activate trial &rarr;',
  trialFooter:'Trial ends after 14 days. No charge.',
  invalidEmail:'Please enter a valid email.',
  inferNoAnswer:'No answer yet.',
  inferNa:'Reading this as N/A.',
  inferNever:'Strong "never" signal — scoring 1.',
  inferAlways:'Strong "always" signal — scoring 5.',
  inferRarely:'Reading a "rarely" tone — scoring 2.',
  inferNoBut:'"No, but" pattern — scoring 2.',
  inferNo:'Reading this as a "no" — scoring 1.',
  inferYesBut:'"Yes but…" pattern — scoring 3.',
  inferYesScope:'Scope-limited "yes" — scoring 3.',
  inferYes:'Reading this as confident "yes" — scoring 4.',
  inferMaybe:'Hedged middle ground — scoring 3.',
  inferDefault:'Defaulting to 3 — adjust if it feels off.'
});
if (QBANK_CFG.ui && typeof QBANK_CFG.ui === 'object') {
  for (var uiK in QBANK_CFG.ui) { if (Object.prototype.hasOwnProperty.call(QBANK_CFG.ui, uiK)) UI[uiK] = QBANK_CFG.ui[uiK]; }
}
function ui(k, vars){
...
function inferScore(text){
  if(!text||!text.trim()) return {score:null,confidence:0,rationale:ui('inferNoAnswer')};
  const t=text.trim().toLowerCase();
  if(/\b(n\/a|na|don'?t know|skip|not applicable)\b/.test(t)) return {score:'na',confidence:.9,rationale:ui('inferNa')};
  if(/\b(never|none|absolutely not|not at all)\b/.test(t)) return {score:1,confidence:.85,rationale:ui('inferNever')};
  if(/\b(always|every time|consistently|systematically)\b/.test(t)) return {score:5,confidence:.85,rationale:ui('inferAlways')};
  if(/\b(rarely|seldom|hardly|occasionally)/.test(t)) return {score:2,confidence:.75,rationale:ui('inferRarely')};
  if(/^no[\s,]+but/.test(t)) return {score:2,confidence:.7,rationale:ui('inferNoBut')};
  if(/^no\b/.test(t)) return {score:1,confidence:.7,rationale:ui('inferNo')};
  if(/^yes[\s,]+(sometimes|kind of|trying|partly|partially|mostly when)/.test(t)) return {score:3,confidence:.7,rationale:ui('inferYesBut')};
  if(/^yes[\s,]+(some teams|in part)/.test(t)) return {score:3,confidence:.7,rationale:ui('inferYesScope')};
  if(/^yes\b/.test(t)) return {score:4,confidence:.6,rationale:ui('inferYes')};
  if(/\b(sometimes|maybe|in progress|working on)\b/.test(t)) return {score:3,confidence:.6,rationale:ui('inferMaybe')};
  return {score:3,confidence:.3,rationale:ui('inferDefault')};
}
...
function qbankBoot(){
  // Auto-route from the page config key (falls back to data-aa-default-id on wrapper)
  const wrap = document.getElementById('aa-app');
  const def = QBANK_CFG.key || (wrap && wrap.getAttribute('data-aa-default-id'));
  if(def && ASSESSMENTS[def] && !location.hash.includes('/run/') && !location.hash.includes('/results/')) {
    location.hash = '#/run/' + def;
  } else { render(); }
}
if (document.readyState === 'complete') { qbankBoot(); }
else { window.addEventListener('load', qbankBoot); }
...
<div class="aa-card">
  <div class="aa-card-badge"><span class="aa-pill">${ui('pillFreeLive')}</span><span style="font-size:11px;color:#64748b;font-weight:600">${ui('cardMinutes',{n:a.minutes})}</span></div>
  <h3>${a.title}</h3>
  <p class="aa-tag">${a.tagline}</p>
  <p class="aa-desc">${a.description.substring(0,140)}…</p>
  <div class="aa-card-meta"><span>${ui('dimensionsCount',{n:a.dimensions.length})}</span><span>${ui('questionsCount',{n:a.questionCount})}</span></div>
  <div><a class="aa-btn aa-btn-primary" href="#/run/${a.id}">${ui('startBtn')}</a></div>
</div>`).join('');
...
<div class="aa-card aa-soon">
  ...
  <div><a class="aa-btn aa-btn-ghost" href="https://meetings.hubspot.com/john2795">${ui('notifyBtn')}</a></div>
</div>`).join('');
  return `<div class="aa-hero"><div class="aa-inner">
<span class="aa-eyebrow" style="display:block;text-align:center;width:fit-content;margin:0 auto">${ui('landingEyebrow')}</span>
<h1>${ui('landingH1')}</h1>
<p class="aa-sub">${ui('landingSub',{n:Object.keys(ASSESSMENTS).length})}</p>
<div class="aa-hero-cta"><a class="aa-btn aa-btn-primary" href="#/run/${Object.keys(ASSESSMENTS)[0]||'innovation-framework'}">${ui('startNowBtn')}</a></div>
<div class="aa-trust-strip"><span>${ui('trust1')}</span><span>·</span><span>${ui('trust2')}</span><span>·</span><span>${ui('trust3')}</span></div>
</div></div>
...
<h2 style="font-size:26px;color:#fff;margin-bottom:10px">${ui('landingUpsellH')}</h2>
<p style="color:#cbd5e1;margin-bottom:22px">${ui('landingUpsellP')}</p>
<button class="aa-btn aa-btn-accent" onclick="AA.signupTrial()">${ui('trialCtaBtn')}</button>
</div></div>`;
}

function startOrResume(id,el){
  if(!ASSESSMENTS[id]) { el.innerHTML='<div style="padding:60px;text-align:center"><h2>'+ui('unknownAssessment')+'</h2><a href="#/" class="aa-btn aa-btn-primary">'+ui('backToAll')+'</a></div>'; return; }
...
function renderRunner(el){
  ...
  if(STATE.stepIndex===0){
    inner+=`<div class="aa-step"><div class="aa-step-meta"><span>${a.title}</span><span>${ui('introMinutes',{n:a.minutes})}</span></div>
...
<span>${ui('dimensionsCount',{n:a.dimensions.length})}</span><span>·</span><span>${ui('questionsCount',{n:a.questionCount})}</span><span>·</span><span>${ui('resultBands')}</span>
</div>
<div style="padding:16px;background:#f8fafc;border-radius:10px;font-size:13px;color:#475569;margin-bottom:20px">
<strong style="color:#053947">${ui('howItWorksLabel')}</strong> ${ui('howItWorksBody')}
</div>
<div class="aa-nav"><a class="aa-btn aa-btn-ghost" href="#/">${ui('allAssessments')}</a><span class="aa-spacer"></span><button class="aa-btn aa-btn-primary" onclick="AA.next()">${ui('beginBtn')}</button></div></div>`;
  } else if(STATE.stepIndex===1){
    const pq=a.perspectiveQuestion;
    inner+=`<div class="aa-step"><div class="aa-step-meta"><span>${ui('calibration')}</span><span>${ui('stepOf',{n:1,total:totalSteps-1})}</span></div>
...
<div class="aa-nav"><button class="aa-btn aa-btn-ghost" onclick="AA.prev()">${ui('backBtn')}</button><span class="aa-spacer"></span></div></div>`;
  } else {
    const qIdx=STATE.stepIndex-2;
    const q=qs[qIdx];
    if(!q){el.innerHTML='<div style="padding:80px;text-align:center"><div style="font-size:50px">🎯</div><h2>'+ui('crunching')+'</h2></div>';setTimeout(()=>submit(),700);return;}
...
<span class="aa-dim-pill" style="background:${q.dim.color}">${q.dim.name}</span><span>${ui('qOf',{n:qIdx+1,total:qs.length})}</span></div>
<h2 class="aa-q-text">${q.text}</h2>${q.probe?`<p class="aa-q-probe">${q.probe}</p>`:''}
<textarea class="aa-input" id="aa-input" placeholder="${ui('answerPlaceholder')}" oninput="AA.onAnswerChange(this.value)">${ans.text||''}</textarea>
${showReflect?`<div class="aa-reflect"><span style="font-size:18px">💬</span><div>${ui('reflectLine',{read:(inf.score==='na'?ui('reflectNa'):ui('reflectScore',{n:inf.score})),rationale:inf.rationale.replace(/—.*/,'')})}</div></div>`:''}
<div class="aa-likert">${[1,2,3,4,5].map(n=>`<button class="${display===n?'aa-sel':''}" onclick="AA.pickScore(${n})">${n}</button>`).join('')}<button class="${display==='na'?'aa-sel aa-sel-na':''}" onclick="AA.pickScore('na')">${ui('naBtn')}</button></div>
<div style="display:flex;justify-content:space-between;font-size:11px;color:#94a3b8;margin-top:8px;text-transform:uppercase;letter-spacing:.6px"><span>${ui('scaleMin')}</span><span>${ui('scaleMax')}</span></div>
<div class="aa-nav"><button class="aa-btn aa-btn-ghost" onclick="AA.prev()">${ui('backBtn')}</button><span class="aa-spacer"></span><button class="aa-btn aa-btn-primary" ${display===null?'disabled':''} onclick="AA.next()">${qIdx===qs.length-1?ui('submitBtn'):ui('nextBtn')}</button></div></div>`;
  }
  inner+='</div>';
...
function renderResults(id){
  ...
  if(!a) return '<div style="padding:60px;text-align:center">'+ui('noAssessment')+'</div>';
  let result=null;try{result=JSON.parse(sessionStorage.getItem('aa_result')||'null');}catch(e){}
  if(!result||result.assessmentId!==id) return `<div style="padding:80px;text-align:center"><h2>${ui('noResultsH')}</h2><p style="margin:16px 0">${ui('noResultsP',{title:a.title})}</p><a class="aa-btn aa-btn-primary" href="#/run/${id}">${ui('startAssessmentBtn')}</a></div>`;
...
<div class="aa-score-hero"><div style="font-size:42px">🎉</div><div class="aa-score-num">${result.total}</div><div class="aa-score-out">${ui('outOf',{n:result.maxDisplay})}</div>
<div class="aa-score-band">${result.band.name}</div><div class="aa-score-head">${result.band.headline}</div></div>
<div class="aa-grid-2">
<div class="aa-panel"><h3>${ui('dimHeading')}</h3>${dimsHtml}<div class="aa-hint"><strong>${ui('diagnosticHint')}</strong> ${result.band.hint||''}</div></div>
<div class="aa-panel"><h3>${ui('shapeHeading')}</h3><div class="aa-radar-wrap">${radarSvg}</div></div>
</div>
<div class="aa-panel" style="margin-bottom:22px"><h3>${ui('meaningHeading')}</h3><p style="font-size:15px;line-height:1.65">${result.band.summary}</p></div>`;
  if(isPaid){
    const plan=a.plans[result.band.id];
    html+=`<div class="aa-plan-grid">
<div class="aa-plan-card"><h4>${ui('plan30')}</h4>${plan.d30.map(x=>`<div class="aa-plan-action"><strong>${x.t}</strong>${x.d}</div>`).join('')}</div>
<div class="aa-plan-card"><h4>${ui('plan60')}</h4>${plan.d60.map(x=>`<div class="aa-plan-action"><strong>${x.t}</strong>${x.d}</div>`).join('')}</div>
<div class="aa-plan-card"><h4>${ui('plan90')}</h4>${plan.d90.map(x=>`<div class="aa-plan-action"><strong>${x.t}</strong>${x.d}</div>`).join('')}</div></div>
<div class="aa-panel"><h3>${ui('readingHeading')}</h3><div class="aa-reading">${plan.reading.map(b=>`<div class="aa-book"><strong>${b.t}</strong>${b.a} · ${b.y}</div>`).join('')}</div></div>
<div style="text-align:center;margin-top:28px"><button class="aa-btn aa-btn-primary" onclick="window.print()">${ui('downloadPdf')}</button> <a class="aa-btn aa-btn-ghost" href="#/">${ui('allAssessments')}</a></div>`;
  } else {
    html+=`<div class="aa-gate"><div class="aa-gate-inner">
<div><span class="aa-eyebrow" style="background:rgba(251,191,36,.2);color:#fbbf24">${ui('gateEyebrow')}</span>
<h2 style="margin-top:14px">${ui('gateH')}</h2>
<p>${ui('gateP')}</p>
<button class="aa-btn aa-btn-accent" onclick="AA.signupTrial()">${ui('trialBtn')}</button> &nbsp; <a class="aa-btn aa-btn-ghost" style="color:#fff;border-color:#fff" href="https://meetings.hubspot.com/john2795">${ui('talkToMark')}</a>
</div>
<div class="aa-pricing"><div style="font-size:11px;color:#fbbf24;font-weight:700;letter-spacing:1px;text-transform:uppercase;margin-bottom:8px">${ui('afterTrial')}</div>
<div class="aa-amt">${ui('priceAmount')}</div><div class="aa-per">${ui('pricePer')}</div>
<ul><li>${ui('priceB1')}</li><li>${ui('readingHeading')}</li><li>${ui('priceB3')}</li><li>${ui('priceB4')}</li><li>${ui('priceB5')}</li></ul></div>
</div></div>`;
  }
...
<p style="font-size:13px;color:#64748b;margin-bottom:10px">${ui('donePrompt')}</p>
<a class="aa-btn aa-btn-ghost" href="#/run/${next.id}">${ui('tryNext',{title:next.title})}</a></div>`;
  }
  html+='</div></div>';
  return html;
}
...
<h2 style="color:#fff;font-size:30px;font-weight:800;margin:0 0 10px">${ui('gateDoneH')}</h2>
<p style="color:#cbd5e1;font-size:16px;max-width:540px;margin:0 auto 22px">${ui('gateDoneP',{title:a.title})}</p>
...
<div class="aa-form-field"><label style="color:#fff">${ui('emailLabelGate')}</label><input type="email" id="aa-gate-email" placeholder="${ui('emailPlaceholder')}" style="background:rgba(255,255,255,0.15);border-color:rgba(255,255,255,0.3);color:#fff"/></div>
<div class="aa-form-field"><label style="color:#fff">${ui('nameLabelGate')}</label><input type="text" id="aa-gate-name" placeholder="${ui('namePlaceholder')}" style="background:rgba(255,255,255,0.15);border-color:rgba(255,255,255,0.3);color:#fff"/></div>
<div id="aa-gate-err" style="font-size:13px;color:#fca5a5;margin-bottom:10px;display:none"></div>
<button class="aa-btn aa-btn-accent" style="width:100%" onclick="AA.unlockResults('${a.id}')">${ui('seeResultsBtn')}</button>
<p style="font-size:11px;color:#cbd5e1;text-align:center;margin-top:10px">${ui('noSpam')}</p>
</div></div></div></div>`;}

/* ---- LEAD CAPTURE -------------------------------------------------------
   Both gates below used to write the address into localStorage and stop
   there. 84KB of engine and not one network call: every email typed into
   any of the five assessments, in any language, stayed on the visitor's
   own machine and was never seen by anyone here. The gate says "No spam.
   Unsubscribe anytime.", which implies a list that did not exist.

   This sends it to HubSpot as well. The localStorage write stays - it is
   what keeps someone's tier across page loads.

   FIRE AND FORGET, DELIBERATELY. Never await this and never let it gate
   render(): a network failure must not stop someone seeing the results
   they just spent ten minutes earning. Every failure path is swallowed.

   The HubSpot form carries email and firstname only, and HubSpot rejects
   an entire submission that names a field the form does not have - the
   email would be lost with it. So the assessment and the event ride in
   context.pageName, which HubSpot stores and shows against the
   submission. pageUri separates the language mirrors on its own. To get
   the score and band into the CRM, add one text property to the contact,
   put it on that form, and extend fields[] here. */
const HS_PORTAL = '46316757';
const HS_FORM   = 'c6f0d4c1-d233-4875-9b0c-4528cda02237';

function captureLead(email, name, event) {
  if (!HS_PORTAL || !HS_FORM || !email) { return; }
  const a = ASSESSMENTS[STATE.currentId] || {};
    ...
    fetch('https://api.hsforms.com/submissions/v3/integration/submit/' + HS_PORTAL + '/' + HS_FORM, {
      ...
      body: JSON.stringify({
        ...
        context: {
          ...
          pageName: (a.title || STATE.currentId || 'Assessment') + ' - ' + event
        }
      })
    }).catch(function () {});
  } catch (_) {}
...
window.AA={
  ...
  signupTrial(){AA.openModal(`<h3 style="font-size:22px;font-weight:800;margin-bottom:8px">${ui('trialModalH')}</h3>
<p style="margin-bottom:16px">${ui('trialModalP')}</p>
<div class="aa-form-field"><label>${ui('emailLabel')}</label><input type="email" id="aa-trial-email" placeholder="${ui('emailPlaceholder')}"/></div>
<div class="aa-form-field"><label>${ui('nameLabel')}</label><input type="text" id="aa-trial-name" placeholder="${ui('namePlaceholder')}"/></div>
<div id="aa-trial-err" style="font-size:13px;color:#dc2626;margin-bottom:10px;display:none"></div>
<button class="aa-btn aa-btn-primary" style="width:100%" onclick="AA.activateTrial()">${ui('activateTrialBtn')}</button>
<p style="font-size:11px;color:#64748b;text-align:center;margin-top:10px">${ui('trialFooter')}</p>`);},
  unlockResults(id){const e=document.getElementById('aa-gate-email').value.trim();const err=document.getElementById('aa-gate-err');if(!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(e)){err.style.display='block';err.textContent=ui('invalidEmail');return;}const n=document.getElementById('aa-gate-name').value.trim();STATE.userEmail=e;try{localStorage.setItem('aa_user',JSON.stringify({tier:'free',email:e,name:n,ts:Date.now()}));}catch(_){}captureLead(e,n,'results unlocked');render();},
  activateTrial(){const e=document.getElementById('aa-trial-email').value.trim();const err=document.getElementById('aa-trial-err');if(!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(e)){err.style.display='block';err.textContent=ui('invalidEmail');return;}const n=document.getElementById('aa-trial-name').value.trim();STATE.userTier='trial';STATE.userEmail=e;try{localStorage.setItem('aa_user',JSON.stringify({tier:'trial',email:e,name:n,ts:Date.now(),trialEnds:Date.now()+14*24*60*60*1000}));}catch(_){}captureLead(e,n,'trial started');AA.closeModal();render();}
};
})();
