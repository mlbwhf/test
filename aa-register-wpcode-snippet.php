...
function aa_reg_months( $cohorts ) {
	...
	foreach ( $cohorts as $c ) {
		...
		if ( ! isset( $by[ $k ] ) ) { $by[ $k ] = array( 'label' => aa_reg_month_label( $c['start'] ), 'items' => array() ); }
		$by[ $k ]['items'][] = $c;
	}
	return $by;
}
...
function aa_reg_panel( $atts ) {
	...
	$h .= '<p class="aacal-eyebrow">' . esc_html( $course['code'] . ' · ' . $course['days'] . ' days' ) . '</p>';
	$h .= '<h2 class="aacal-h2" id="aacal-title">Choose a batch</h2>';
	...
	$i = 0;
	foreach ( $months as $k => $m ) {
		$h .= '<div class="aacal-grid" role="tabpanel" id="aacal-panel-' . esc_attr( $k ) . '"'
		    . ' aria-labelledby="aacal-tab-' . esc_attr( $k ) . '" data-month="' . esc_attr( $k ) . '"'
		    . ( $i === 0 ? '' : ' hidden' ) . '>';
		foreach ( $m['items'] as $c ) {
			$on   = $c['id'] === $first['id'];
			$left = aa_reg_seats_left( $course, $c );
			$hot  = $left <= 6;
			$h .= '<article class="aacal-card' . ( $on ? ' is-on' : '' ) . '" data-cohort="' . esc_attr( $c['id'] ) . '"'
			    ...
			    . '<button type="button" class="aacal-cardbtn" aria-pressed="' . ( $on ? 'true' : 'false' ) . '">'
			    . ( $on ? '<span class="aacal-flag">Next available</span>' : '' )
			    . '<span class="aacal-cardtop">' . esc_html( aa_reg_range( $c['start'], $c['end'], true ) ) . '</span>'
			    . '<span class="aacal-batch">' . esc_html( $c['batch'] . ' · ' . $c['hours'] ) . '</span>'
			    . '<span class="aacal-cardfoot"><span class="aacal-status' . ( $hot ? ' is-hot' : '' ) . '">'
			    . esc_html( $hot ? sprintf( '%d seats left', $left ) : 'Seats open' ) . '</span>'
			    . '<span class="aacal-price">' . esc_html( aa_reg_money( $course['price'], $cur ) ) . '</span></span>'
			    . '</button></article>';
		}
		$h .= '</div>';
		$i++;
	...
	$h .= '<div class="aacal-sel" aria-live="polite"><p class="aacal-sellabel" data-sel-label>Selected · next available</p>'
	    . '<p class="aacal-selrange" data-sel-range>' . esc_html( aa_reg_range( $first['start'], $first['end'] ) ) . '</p>'
	    . '<p class="aacal-selbatch" data-sel-batch>' . esc_html( $first['batch'] ) . '</p></div>';

	$h .= '<form class="aacal-panel" data-panel="1" novalidate>'
	    . '<div class="aacal-two"><label class="aacal-vh" for="aa-first">First name</label>'
	    . '<input id="aa-first" name="first" placeholder="First name" autocomplete="given-name" required>'
	    . '<label class="aacal-vh" for="aa-last">Last name</label>'
	    . '<input id="aa-last" name="last" placeholder="Last name" autocomplete="family-name" required></div>'
	    . '<label class="aacal-vh" for="aa-email">Work email</label>'
	    . '<input id="aa-email" name="email" type="email" placeholder="Work email" autocomplete="email" required>'
	    . '<div class="aacal-two"><label class="aacal-vh" for="aa-company">Company</label>'
	    . '<input id="aa-company" name="company" placeholder="Company" autocomplete="organization">'
	    . '<label class="aacal-vh" for="aa-phone">Phone (optional)</label>'
	    . '<input id="aa-phone" name="phone" placeholder="Phone (optional)" autocomplete="tel"></div>'
	...
	$h .= '<p class="aacal-policy">Free reschedule up to 10 days before the batch starts. The exam fee is included in the price.</p>';
	$h .= '</div></div>';
